...
bool ToHash160(const std::string& strSctcAddress, std::string& strHash160)
{
    CBitcoinAddress sctcAddress(strSctcAddress);
    if(sctcAddress.IsValid()){
        CKeyID keyid;
        sctcAddress.GetKeyID(keyid);
...
bool ToSctcAddress(const std::string& strHash160, std::string& strSctcAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress sctcAddress;
    sctcAddress.Set(keyid);
    if(sctcAddress.IsValid()){
        strSctcAddress = sctcAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToSctcAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToSctcAddress(tokenEvent.receiver, tokenEvent.receiver);
