...
const char * const BITCOIN_CONF_FILENAME = "sctc.conf";
const char * const BITCOIN_PID_FILENAME = "sctcd.pid";
...
void OpenDebugLog()
{
    ...
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // sctc
    assert(vMsgsBeforeOpenLog);
    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // sctc
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // sctc
    if (fileout) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Sctc
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Sctc
    // Mac: ~/Library/Application Support/Sctc
    // Unix: ~/.sctc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sctc";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Sctc";
#else
    // Unix
    return pathRet / ".sctc";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Sctc Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Sctc Core developers";
    }
