...
{
    if (pindexBestHeader->nHeight >= Params().GetConsensus().nLastPOWBlock)//tonisctc edit
        return 0;
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // sctc
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // sctc
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // sctc
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // sctc
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress sctcSenderAddress(request.params[2].get_str());
        if(sctcSenderAddress.IsValid()){
            CKeyID keyid;
            sctcSenderAddress.GetKeyID(keyid);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of sctc addresses\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "callcontract",           &callcontract,           true,  {"address","data"} }, // sctc
