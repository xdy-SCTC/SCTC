...
void initState(){
    ...
    const std::string dirSctc = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<SctcState>(new SctcState(dev::u256(0), SctcState::openDB(dirSctc, hashDB, dev::WithExisting::Trust), dirSctc + "/sctcDB", dev::eth::BaseState::Empty));
...
SctcTransaction createSctcTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    SctcTransaction txEth;
    if(recipient == dev::Address()){
        txEth = SctcTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = SctcTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<SctcTransaction> txs){
