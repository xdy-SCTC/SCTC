...
void checkResult(bool isCreation, std::vector<SctcTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    SctcTxConverter converter(transaction, NULL);
    ExtractSctcTX sctcTx;
    BOOST_CHECK(converter.extractionSctcTransactions(sctcTx));
    std::vector<SctcTransaction> result = sctcTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(sctctxconverter_tests, TestingSetup)
