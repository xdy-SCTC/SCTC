...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/sctcproject/sctc>";
    const std::string URL_WEBSITE = "<https://sctc.org>";
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "stateSctc";
                ...
                boost::filesystem::path sctcStateDir = GetDataDir() / "stateSctc";

                bool fStatus = boost::filesystem::exists(sctcStateDir);
                const std::string dirSctc(sctcStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsSctcstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<SctcState>(new SctcState(dev::u256(0), SctcState::openDB(dirSctc, hashDB, dev::WithExisting::Trust), dirSctc, existsSctcstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::sctcMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(sctcStateDir.string());
