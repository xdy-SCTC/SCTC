...
#if defined(NDEBUG)
# error "Sctc cannot be compiled without assertions."
#endif
...
std::unique_ptr<SctcState> globalState;
...
const std::string strMessageMagic = "Sctc Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // sctc
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            SctcDGP sctcDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = sctcDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = sctcDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            SctcTxConverter converter(tx, NULL);
            ExtractSctcTX resultConverter;
            if(!converter.extractionSctcTransactions(resultConverter)){
            ...
            std::vector<SctcTransaction> sctcTransactions = resultConverter.first;
            std::vector<EthTransactionParams> sctcETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(SctcTransaction sctcTransaction : sctcTransactions){
                sumGas += sctcTransaction.gas() * sctcTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, sctcTransaction.gasPrice());
                } else {
                    txMinGasPrice = sctcTransaction.gasPrice();
                }
                VersionVM v = sctcTransaction.getVersion();
                if(v.format!=0)
                ...
                if(sctcTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(sctcTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(sctcTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)sctcTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(sctcETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > sctcTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s SCTC additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // sctc
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // sctc
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    SctcTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<SctcTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path sctcDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(sctcDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(sctcDir.string(), std::ios::in | std::ios::out);
...
bool SctcTxConverter::extractionSctcTransactions(ExtractSctcTX& sctctx){
    std::vector<SctcTransaction> resultTX;
    ...
    sctctx = std::make_pair(resultTX, resultETP);
    return true;
}

bool SctcTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool SctcTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
SctcTransaction SctcTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    SctcTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = SctcTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = SctcTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // sctc
    SctcDGP sctcDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setSctcSchedule(sctcDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = sctcDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = sctcDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = sctcDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // sctc
...
///////////////////////////////////////////////////////////////////////////////////////// sctc
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            SctcTxConverter convert(tx, &view, &block.vtx);

            ExtractSctcTX resultConvertSctcTX;
            if(!convert.extractionSctcTransactions(resultConvertSctcTX)){
            ...
            if(!CheckMinGasPrice(resultConvertSctcTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertSctcTX.first, blockGasLimit);
            ...
            for(SctcTransaction& qtx : resultConvertSctcTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertSctcTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertSctcTX.first[k].from(), resultConvertSctcTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // sctc
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // sctc
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // sctc
            globalState->setRootUTXO(oldHashUTXORoot); // sctc
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // sctc
        uint32_t sizeBlockDGP = sctcDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Sctc PoS we need to sync this after every block to ensure txdb is populated for
