...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setSctcSchedule(sctcDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = sctcDGP.getBlockSize(nHeight);
    minGasPrice = sctcDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = sctcDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    SctcTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractSctcTX resultConverter;
    if(!convert.extractionSctcTransactions(resultConverter)){
    ...
    for(SctcTransaction sctcTransaction : sctcTransactions){
        txGas += sctcTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + sctcTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(sctcTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, sctcTransactions, hardBlockGasLimit);
...
void StakeSctcs(bool fStake, CWallet *pwallet)
{
