...
bool SctcDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule SctcDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t SctcDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t SctcDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t SctcDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t SctcDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool SctcDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void SctcDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void SctcDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void SctcDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void SctcDGP::createParamsInstance(){
...
dev::Address SctcDGP::getAddressForBlock(unsigned int blockHeight){
...
void SctcDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SctcDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void SctcDGP::parseStorageOneUint64(uint64_t& value){
...
void SctcDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule SctcDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
