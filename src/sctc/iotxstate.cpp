...
SctcState::SctcState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = SctcState::openDB(_path + "/sctcDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

SctcState::SctcState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute SctcState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, SctcTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createSctcAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            sctc::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> SctcState::vins() const // temp
{
...
void SctcState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* SctcState::vin(dev::Address const& _a) const
{
    return const_cast<SctcState*>(this)->vin(_a);
}

Vin* SctcState::vin(dev::Address const& _addr)
{
...
// void SctcState::commit(CommitBehaviour _commitBehaviour)
// {
...
void SctcState::kill(dev::Address _addr)
{
...
void SctcState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address SctcState::createSctcAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void SctcState::deleteAccounts(std::set<dev::Address>& addrs){
...
void SctcState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void SctcState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
