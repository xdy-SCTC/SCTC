...
*Setup instructions for a Gitian build of Sctc Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sctc
...
- [Building Sctc Core](#building-sctc-core)
...
Clone the git repositories for sctc and Gitian.
...
```bash
sudo cp /home/debian/sctc/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the sctc repository under 'Fetch and create inputs' to install sources which require
...
To build Sctc Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the sctc repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sctc/.git/
...
For example:
```bash
URL=https://github.com/laanwj/sctc.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit sctc=${COMMIT} --url sctc=${URL} ../sctc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sctc=${COMMIT} --url sctc=${URL} ../sctc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sctc=${COMMIT} --url sctc=${URL} ../sctc/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-sctc:
./bin/gbuild --commit sctc=${COMMIT},cpp-eth-sctc=${ETHCOMMIT} --url sctc=${URL},cpp-eth-sctc=${ETHURL} ../sctc/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the sctc git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sctc/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/sctc
SIGPATH=/some/root/path/sctc-detached-sigs

./bin/gbuild --url sctc=${BTCPATH},signature=${SIGPATH} ../sctc/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sctc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sctc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sctc-osx-build.assert
```
...
[sctcproject/gitian.sigs](https://github.com/sctcproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@sctc.org) and he will commit them.
